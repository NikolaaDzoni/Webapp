...
class QuizApp {
    ...
    loadQuestion() {
        ...
        if (question.type === 'fill-in') {
            ...
            // Ensure userAnswers structure exists for this question and is initialized with current input values or empty strings
            if (!this.userAnswers[question.id]) {
                 this.userAnswers[question.id] = {
                      selectedAnswer: Array.from(inputElements).map(input => String(input.value).trim()), // Initialize from current input values
                      awardedPoints: 0,
                      answerRevealed: false
                 };
            } else {
                 // Ensure selectedAnswer array has the correct size and populate with current input values
                 const currentInputValues = Array.from(inputElements).map(input => String(input.value).trim());
                 this.userAnswers[question.id].selectedAnswer = currentInputValues;

                 // Ensure other properties exist if not present (shouldn't happen with proper initialization, but for safety)
                 if (this.userAnswers[question.id].awardedPoints === undefined) this.userAnswers[question.id].awardedPoints = 0;
                 if (this.userAnswers[question.id].answerRevealed === undefined) this.userAnswers[question.id].answerRevealed = false;
            }

            inputElements.forEach((inputElement, index) => {
                 // Restore user's previous input if available, ensuring it's a string
                 if (this.userAnswers[question.id].selectedAnswer[index] !== undefined) {
                     ...
                     inputElement.value = ''; // Ensure input is cleared if no previous answer
                 }
                ...
                inputElement.addEventListener('input', () => {
                    // Update the specific input value in the userAnswer array, trimmed
                    this.userAnswers[question.id].selectedAnswer[index] = inputElement.value.trim(); // Add trim() here

                    // Apply coloring and update indicator for only the input that changed
                    // We re-apply feedback for all inputs to ensure consistency, but the logic inside handles each individually
                    this.applyFeedbackColoring(question, this.userAnswers[question.id]);
                 ...
                 inputElement.addEventListener('blur', () => {
                     // Update the specific input value in the userAnswer array on blur as well, trimmed
                      this.userAnswers[question.id].selectedAnswer[index] = inputElement.value.trim(); // Add trim() here

                     // Recalculate points for the entire question based on all inputs
        ...
        if (userAnswer) {
                ...
                optionElement.addEventListener('click', () => {
                     ...
                     inputElement.checked = !inputElement.checked; // Toggle checked state for checkbox, always check for radio
                     if (!isMultipleChoice) inputElement.checked = true; // Ensure radio button gets checked on click
    ...
    handleRadioChange(questionId, selectedIndex) {
        ...
        this.userAnswers[questionId] = {
            ...
            awardedPoints: this.checkAnswerCorrectness(question, selectedIndex), // Store awarded points
            answerRevealed: false // Reset answerRevealed on new selection
        };
        this.updateProgress();
        // Coloring is now handled by the click event listener calling applyFeedbackColoring
    }
    ...
    checkAnswerCorrectness(question, selectedAnswer) {
        if (question.type === 'fill-in') {
            ...
            for (let i = 0; i < totalBlanks; i++) {
                 // --- CORRECTED: Get the specific inputElement for the current index i ---
                 const inputElement = inputElements[i];
                 // --- END CORRECTED ---

                 const rawCorrectAnswer = inputElement.getAttribute('data-correct'); // Use the correct inputElement here
                 const userAnswer = selectedAnswer[i];

                 // Decode HTML entities in the correct answer for accurate comparison
                 const tempElement = document.createElement('div');
                 tempElement.innerHTML = rawCorrectAnswer;
                 const decodedCorrectAnswer = tempElement.textContent;

                 // --- CORRECTED CONSOLE.LOGS (using the local inputElement) ---
                 console.log(`Q${question.id} Input ${i+1}:`);
                 console.log(`  User Input (raw): "${inputElement.value}"`);
                 console.log(`  User Input (from selectedAnswer, trimmed, lowercased): "${String(userAnswer).trim().toLowerCase()}"`); // Log from selectedAnswer
                 console.log(`  Correct Answer (raw data-correct): "${rawCorrectAnswer}"`);
                 console.log(`  Correct Answer (decoded, trimmed, lowercased): "${String(decodedCorrectAnswer).trim().toLowerCase()}"`);
                 console.log(`  Comparison Result: ${String(userAnswer).trim().toLowerCase() === String(decodedCorrectAnswer).trim().toLowerCase()}`);
                 // --- END CORRECTED CONSOLE.LOGS ---


                 // Check if user answer exists, is not empty, and matches correct answer (case-insensitive and trimmed)
                 // Ensure both sides are treated as strings and trimmed before comparison
                 if (String(userAnswer).trim().toLowerCase() === String(decodedCorrectAnswer).trim().toLowerCase()) {
                      correctlyFilledCount++;
                 }
            }

            // Award points proportionally only if all correct blanks are filled correctly
            // If any blank is incorrect or left empty, award 0 points based on the clarification
    ...
    applyFeedbackColoring(question, userAnswer) {
        ...
        if (!userAnswer || !userAnswer.selectedAnswer || !Array.isArray(userAnswer.selectedAnswer)) {
             // If no answer, or selectedAnswer is not a valid array, just clear coloring/indicators and return
             return;
        }
